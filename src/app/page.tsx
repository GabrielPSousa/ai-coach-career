...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'resume' | 'match' | 'plan' | 'interview'>('dashboard');
  ...
  } = (useChat as unknown as (options: Record<string, unknown>) => {
    messages: { id: string, role: string, content: string }[],
    input: string,
    handleInputChange: (e: React.ChangeEvent<HTMLInputElement>) => void,
    handleSubmit: (e: React.FormEvent<HTMLFormElement> | undefined) => void,
    setMessages: (messages: { id: string, role: string, content: string }[]) => void,
    append: (message: { role: 'user' | 'assistant' | 'system' | 'data', content: string }) => Promise<string | null | undefined>,
    isLoading: boolean,
  })({
  ...
  const handleSyncKnowledge = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro de rede ao conectar com o serviço de ingestão';
      setSyncResult({
        success: false,
  ...
  const handleJobMatchSubmit = async () => {
    if (!resumeText) {
      alert('Por favor, faça o upload ou cole seu currículo na aba de "Currículo" primeiro!');
      setActiveTab('resume');
      return;
    }
    ...
    try {
      const res = await fetch('/api/learning-plan', {
        ...
        body: JSON.stringify({
          ...
          resumeSummary: resumeAnalysis ? `Candidato de nível ${resumeAnalysis.seniority}` : undefined
        }),
      });
  ...
  const handleStartInterview = async () => {
    ...
    // Usamos o método append oficial do Vercel AI SDK para fazer isso de forma limpa e assíncrona.
    setTimeout(() => {
      append({
        role: "user",
        content: "Estou pronto para iniciar a simulação de entrevista técnica."
      });
    }, 200);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">RAG-Powered AI Mentoring & Ingestion Platform</p>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-8 flex flex-col gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              <div className="relative overflow-hidden bg-gradient-to-br from-indigo-900/40 via-slate-900 to-slate-950 p-8 rounded-3xl border border-slate-800 shadow-xl">
                ...
                <div className="relative flex flex-col gap-4">
                  ...
                  <p className="text-slate-300 text-sm md:text-base leading-relaxed max-w-2xl">
                    Utilize o poder das tecnologias mais avançadas de inteligência artificial (RAG com banco vetorial pgvector no Supabase e modelos GPT-4o) para analisar seus gaps, traçar planos de estudo perfeitos e simular desafios de entrevistas reais.
                  </p>
              ...
              <div className="bg-slate-900/40 border border-slate-850 p-6 rounded-2xl flex flex-col gap-4">
                <h3 className="font-bold text-lg text-white flex items-center gap-2">
                  <Layers className="w-5 h-5 text-indigo-400" /> Fluxo de Crescimento Técnico
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-2">
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col gap-2">
                    <div className="bg-indigo-500/10 text-indigo-400 font-bold text-xs w-6 h-6 rounded-full flex items-center justify-center">1</div>
                    <h4 className="font-semibold text-sm text-slate-200">Suba o Currículo</h4>
                    <p className="text-xs text-slate-400">Analise suas habilidades e obtenha sua senioridade do mercado.</p>
                  </div>
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col gap-2">
                    <div className="bg-indigo-500/10 text-indigo-400 font-bold text-xs w-6 h-6 rounded-full flex items-center justify-center">2</div>
                    <h4 className="font-semibold text-sm text-slate-200">Escolha a Vaga</h4>
                    <p className="text-xs text-slate-400">Insira a descrição da vaga desejada para calcular o score.</p>
                  </div>
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col gap-2">
                    <div className="bg-indigo-500/10 text-indigo-400 font-bold text-xs w-6 h-6 rounded-full flex items-center justify-center">3</div>
                    <h4 className="font-semibold text-sm text-slate-200">Plano de Estudos</h4>
                    <p className="text-xs text-slate-400">Gere um cronograma baseado em RAG para as skills ausentes.</p>
                  </div>
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col gap-2">
                    <div className="bg-indigo-500/10 text-indigo-400 font-bold text-xs w-6 h-6 rounded-full flex items-center justify-center">4</div>
                    <h4 className="font-semibold text-sm text-slate-200">Simule Entrevistas</h4>
                    <p className="text-xs text-slate-400">Treine suas respostas com recrutadores virtuais exigentes.</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-white flex items-center gap-2 text-sm">
                    <Cpu className="w-4 h-4 text-emerald-400" /> Base de Conhecimento RAG
                  </h3>
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Nossa base de conhecimento local possui arquivos markdown categorizados sobre <b>React, Node.js, AWS, PostgreSQL, System Design</b> e <b>Behavioral</b>. Ao sincronizar, o sistema cria representações vetoriais de 1536 dimensões via OpenAI Embeddings no Supabase PostgreSQL.
                </p>
                ...
                >
                  ...
                  {isSyncingKnowledge ? 'Sincronizando Base...' : 'Sincronizar Arquivos Locais'}
                </button>

                {/* RESULTADOS DA SINCRO */}
                {syncResult && (
                  <div className={`mt-2 p-3.5 rounded-xl text-xs border ${
                    syncResult.success 
                    ...
                    {syncResult.success ? (
                      <div className="flex flex-col gap-1">
                        <p className="font-bold flex items-center gap-1.5">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Sincronização Concluída!
                        </p>
                        <ul className="list-disc pl-4 mt-1 flex flex-col gap-0.5 text-[11px] text-slate-300">
                          <li>Documentos lidos: {syncResult.processedDocuments}</li>
                      ...
                      <div className="flex flex-col gap-1">
                        <p className="font-bold flex items-center gap-1.5">
                          <AlertTriangle className="w-3.5 h-3.5 text-red-400" /> Falhas encontradas:
                        </p>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
                ...
                {resumeAnalysis ? (
                  <div className="flex flex-col gap-3">
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-850 flex items-center justify-between">
                      <div className="truncate pr-2">
                        <p className="text-xs font-semibold text-slate-300 truncate">{resumeFilename || 'curriculo.txt'}</p>
                        <p className="text-[10px] text-slate-500">Pronto para Match e Estudos</p>
                      </div>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-[11px] text-slate-400">Competências:</span>
                      <div className="flex flex-wrap gap-1 max-h-16 overflow-hidden">
                        ...
                        {resumeAnalysis.skills.length > 5 && (
                          <span className="text-[10px] text-slate-500 pl-1">+{resumeAnalysis.skills.length - 5}</span>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-800 text-[11px]">
                      ...
                      <button 
                        onClick={() => handleGenerateStudyPlan(resumeAnalysis.weaknesses)}
                  ...
                  <div className="text-center py-6 flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs text-slate-400">Nenhum currículo ativo carregado nesta sessão.</p>
                    <button 
                      onClick={() => setActiveTab('resume')}
                      className="text-indigo-400 font-bold text-xs hover:underline flex items-center gap-1"
                    >
                      Carregar meu currículo <ArrowRight className="w-3.5 h-3.5" />
                    </button>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
                <FileText className="w-6 h-6 text-indigo-400" /> Analisador de Currículo (Agent 1)
              </h2>
              <p className="text-slate-400 text-sm">
                Envie seu currículo nos formatos <b>PDF</b>, <b>DOCX</b> ou <b>Texto</b>. Nosso agente extrairá suas habilidades técnicas e soft skills de forma automatizada para desenhar sua trilha profissional.
              </p>
            </div>
                  ...
                  >
                    ...
                    {isAnalyzingResume ? (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-200">Extraindo e analisando currículo...</p>
                          <p className="text-xs text-slate-400 mt-1">Isso pode levar de 5 a 15 segundos</p>
                        </div>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-200">Clique para selecionar seu arquivo</p>
                          <p className="text-xs text-slate-400 mt-1">PDF, DOCX ou TXT de até 10MB</p>
                        </div>
                        {resumeFilename && (
                ...
                {resumeText && (
                  <div className="bg-slate-900 border border-slate-850 rounded-2xl p-4">
                    <h4 className="text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider flex items-center justify-between">
                      ...
                      <button 
                        onClick={() => { setResumeText(''); setResumeAnalysis(null); setResumeFilename(''); }}
                ...
                {resumeAnalysis ? (
                  <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col gap-6 shadow-lg">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-extrabold text-lg text-white">Análise de Perfil Executada</h3>
                          <p className="text-xs text-slate-400">Resultados estruturados extraídos pelo Agent 1</p>
                        </div>
                      </div>
                      
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-400">Senioridade Atribuída:</span>
                    ...
                    <div className="flex flex-col gap-3">
                      <h4 className="font-bold text-slate-200 text-sm">Habilidades & Tecnologias Dominadas:</h4>
                      <div className="flex flex-wrap gap-1.5">
                        {resumeAnalysis.skills.map((skill, i) => (
                          <span key={i} className="bg-slate-800 text-slate-200 border border-slate-700/60 text-xs px-3 py-1 rounded-lg font-medium">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                      ...
                      <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-2xl p-5 flex flex-col gap-3">
                        <h4 className="font-bold text-emerald-400 text-sm flex items-center gap-2">
                          <CheckCircle2 className="w-4.5 h-4.5" /> Pontos Fortes do Perfil
                        </h4>
                      ...
                      <div className="bg-amber-500/5 border border-amber-500/15 rounded-2xl p-5 flex flex-col gap-3">
                        <h4 className="font-bold text-amber-400 text-sm flex items-center gap-2">
                          <AlertTriangle className="w-4.5 h-4.5" /> Gaps Técnicos / Oportunidades
                        </h4>
                        <ul className="flex flex-col gap-2.5 text-xs text-slate-300">
                    ...
                    <div className="border-t border-slate-800 pt-5 flex flex-wrap gap-3 items-center justify-between">
                      <div className="text-xs text-slate-400 max-w-sm">
                        Com base nos gaps técnicos identificados, você pode gerar um cronograma de estudos sob medida com nossa base de conhecimento (RAG).
                      </div>
                      <button 
                        onClick={() => handleGenerateStudyPlan(resumeAnalysis.weaknesses)}
                        disabled={isGeneratingPlan}
                      ...
                      >
                        {isGeneratingPlan ? (
                          <>
                            <RefreshCw className="w-4.5 h-4.5 animate-spin" /> Gerando Plano...
                          </>
                        ) : (
                          <>
                            Gerar Plano de Estudos RAG <ArrowRight className="w-4 h-4" />
                          </>
                        )}
                      </button>
                    </div>
                  ...
                  <div className="bg-slate-900/30 border border-slate-850 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-4 h-full min-h-80">
                    ...
                    <div className="max-w-md">
                      <h3 className="font-bold text-white text-base">Aguardando Envio de Currículo</h3>
                      <p className="text-xs text-slate-500 mt-1">
                        Faça o upload do seu arquivo de currículo técnico do lado esquerdo para extrair suas competências e classificações do Agent 1.
                      </p>
                    </div>
                  </div>
                )}
        ...
        {activeTab === 'match' && (
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
                <Briefcase className="w-6 h-6 text-indigo-400" /> Comparador de Vagas (Agent 2)
              </h2>
              <p className="text-slate-400 text-sm">
                Forneça a descrição e requisitos da vaga para a qual você deseja aplicar. Nosso agente calculará o score de adequação de forma cirúrgica e identificará o que falta no seu currículo.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
                  <h3 className="font-bold text-slate-200 text-sm border-b border-slate-800 pb-2">Informações da Vaga</h3>
                  
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Título do Cargo*</label>
                    <input 
                      type="text" 
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Empresa (Opcional)</label>
                    <input 
                      type="text" 
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[11px] font-bold text-slate-400 uppercase">Descrição Bruta da Vaga*</label>
                    <textarea 
                  ...
                  >
                    {isMatchingJob ? (
                      <>
                        <RefreshCw className="w-4 h-4 animate-spin" /> Calculando Match...
                      </>
                    ) : (
                      <>
                        Verificar Compatibilidade <Sparkles className="w-4 h-4" />
                      </>
                    )}
                  </button>
                </div>
              ...
              <div className="lg:col-span-2">
                {jobMatchResult ? (
                  <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col gap-6 shadow-lg">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-extrabold text-lg text-white">{jobTitle}</h3>
                          <p className="text-xs text-slate-400">{jobCompany || 'Empresa Informada'}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 bg-slate-950 p-3 rounded-2xl border border-slate-850">
                        <div className="relative w-12 h-12 flex items-center justify-center">
                          ...
                          <span className="text-sm font-black text-white">{jobMatchResult.score}%</span>
                        </div>
                        <div>
                          <p className="text-[10px] text-slate-500 uppercase font-black">Aderência</p>
                          <p className="text-xs font-semibold text-slate-300">
                            {jobMatchResult.score >= 80 ? 'Excelente Fit!' : jobMatchResult.score >= 50 ? 'Bom Potencial' : 'Gaps Críticos'}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-slate-950 p-5 rounded-2xl border border-slate-850 flex flex-col gap-3">
                        <h4 className="font-bold text-emerald-400 text-xs uppercase tracking-wider flex items-center gap-1.5">
                          <CheckCircle2 className="w-4.5 h-4.5 text-emerald-500" /> Skills Alinhadas ({jobMatchResult.matchingSkills.length})
                        </h4>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {jobMatchResult.matchingSkills.map((skill, i) => (
                            <span key={i} className="bg-emerald-500/10 text-emerald-300 border border-emerald-500/15 text-xs px-2.5 py-1 rounded-lg font-medium">
                              {skill}
                            </span>
                          ))}
                          {jobMatchResult.matchingSkills.length === 0 && (
                            <p className="text-xs text-slate-500 italic">Nenhuma skill perfeitamente coincidente mapeada.</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-950 p-5 rounded-2xl border border-slate-850 flex flex-col gap-3">
                        <h4 className="font-bold text-red-400 text-xs uppercase tracking-wider flex items-center gap-1.5">
                          <AlertTriangle className="w-4.5 h-4.5 text-red-500" /> Skills Ausentes / Fracas ({jobMatchResult.missingSkills.length})
                        </h4>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {jobMatchResult.missingSkills.map((skill, i) => (
                            <span key={i} className="bg-red-500/10 text-red-300 border border-red-500/15 text-xs px-2.5 py-1 rounded-lg font-medium">
                              {skill}
                            </span>
                          ))}
                          {jobMatchResult.missingSkills.length === 0 && (
                            <p className="text-xs text-emerald-400 font-bold italic">Nenhum gap detectado!</p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-3 mt-1">
                      <h4 className="font-bold text-slate-200 text-sm">Ações Corretivas Recomendadas:</h4>
                      <div className="grid grid-cols-1 gap-2.5">
                        {jobMatchResult.recommendations.map((rec, i) => (
                          <div key={i} className="bg-slate-950/60 p-4 rounded-xl border border-slate-850 flex gap-3 text-xs leading-relaxed text-slate-300">
                    ...
                    {jobMatchResult.missingSkills.length > 0 && (
                      <div className="border-t border-slate-800 pt-5 flex flex-wrap gap-3 items-center justify-between">
                        <div className="text-xs text-slate-400 max-w-sm">
                          Deseja acelerar a correção dessas lacunas técnicas? O Agent 3 pode criar uma trilha integrada baseada em RAG no nosso banco.
                        </div>
                        <button 
                          onClick={() => handleGenerateStudyPlan(jobMatchResult.missingSkills)}
                          disabled={isGeneratingPlan}
                          className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs transition-all flex items-center gap-2 cursor-pointer shadow-md"
                        >
                          {isGeneratingPlan ? (
                            <>
                              <RefreshCw className="w-4 h-4 animate-spin" /> Gerando Plano...
                            </>
                          ) : (
                            <>
                              Gerar Trilha de Estudos para Gaps <BookOpen className="w-4 h-4" />
                            </>
                          )}
                        </button>
                      </div>
                  ...
                  <div className="bg-slate-900/30 border border-slate-850 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-4 h-full min-h-80">
                    ...
                    <div className="max-w-md">
                      <h3 className="font-bold text-white text-base">Pronto para Análise Comparativa</h3>
                      <p className="text-xs text-slate-500 mt-1">
                        Preencha o formulário com a vaga desejada à esquerda. Nós buscaremos seu currículo carregado automaticamente para avaliar seu encaixe.
                      </p>
                    </div>
                  </div>
                )}
        ...
        {activeTab === 'plan' && (
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
                <BookOpen className="w-6 h-6 text-indigo-400" /> Plano de Estudos Personalizado (Agent 3 - RAG)
              </h2>
              <p className="text-slate-400 text-sm">
                Cronograma semanal estruturado por IA de forma dinâmica. O agente utiliza a busca vetorial para ler nossa base de conhecimento interna e garantir que seu material de apoio seja altamente qualificado e referenciado.
              </p>
            </div>
            ...
            {!learningPlan && (
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-2xl mx-auto text-center flex flex-col items-center gap-5 shadow-lg">
                ...
                <div>
                  <h3 className="font-bold text-white text-lg">Criar Trilha Manual</h3>
                  <p className="text-xs text-slate-400 mt-1.5 max-w-md">
                    Se você não possui dados de match ou quer forçar um estudo personalizado sobre um conjunto específico de competências, digite-as abaixo separadas por vírgula.
                  </p>
                </div>
                <div className="w-full flex gap-2 max-w-md">
                  ...
                  <button 
                    onClick={() => {
                      const inputVal = (document.getElementById('manual-skills') as HTMLInputElement)?.value;
                      if (!inputVal) {
                        alert('Digite pelo menos uma habilidade.');
                        return;
                      }
                      const skills = inputVal.split(',').map(s => s.trim()).filter(Boolean);
            ...
            {isGeneratingPlan && (
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-5 max-w-lg mx-auto shadow-xl">
                <RefreshCw className="w-10 h-10 text-indigo-400 animate-spin" />
                <div>
                  <h3 className="font-bold text-white text-base">Gerando Plano RAG...</h3>
                  <p className="text-xs text-slate-400 mt-1.5 leading-relaxed">
                    Pesquisando na base de dados PostgreSQL pgvector, recuperando blocos de conhecimento de alto valor e orquestrando o modelo estruturado. Por favor, aguarde.
                  </p>
                </div>
              </div>
            )}
            ...
            {learningPlan && !isGeneratingPlan && (
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3 flex flex-col gap-6">
                  <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col gap-8 shadow-md">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                      <div>
                        <h3 className="font-extrabold text-white text-lg">Seu Cronograma Semanal de Estudos</h3>
                        <p className="text-xs text-slate-400">Trilha de estudos focada e otimizada por Inteligência Artificial</p>
                      </div>
                        ...
                        <div key={idx} className="flex gap-4 md:gap-6 relative group">
                          ...
                          <div className="bg-slate-950 p-5 rounded-2xl border border-slate-850 flex-1 flex flex-col gap-4">
                            ...
                            <div>
                              ...
                              <h4 className="font-black text-base text-white mt-1">{week.topic}</h4>
                            ...
                            <div className="flex flex-col gap-1.5">
                              <span className="text-[11px] font-bold text-indigo-400 uppercase tracking-wider">Tarefas Ativas</span>
                              <ul className="flex flex-col gap-2">
                                {week.tasks.map((task, i) => (
                                  <li key={i} className="flex gap-2.5 text-xs text-slate-300 leading-relaxed bg-slate-900/40 p-2.5 rounded-xl border border-slate-900/60">
                                    <span className="bg-slate-800 text-indigo-400 font-black text-[9px] w-4 h-4 rounded-md flex items-center justify-center shrink-0">✓</span>
                                    <span>{task}</span>
                                  </li>
                                ))}
                              </ul>
                            ...
                            {week.resources.length > 0 && (
                              <div className="flex flex-col gap-1.5 pt-3 border-t border-slate-900">
                                <span className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider">Fontes & Materiais de Apoio</span>
                                <div className="flex flex-wrap gap-1.5">
                                  {week.resources.map((res, i) => (
                                    <span key={i} className="bg-emerald-500/5 text-emerald-300 border border-emerald-500/10 text-[10px] px-2.5 py-1 rounded-md font-medium flex items-center gap-1">
                                      <CheckCircle2 className="w-3 h-3 text-emerald-400" /> {res}
                                    </span>
                                  ))}
                                </div>
                              </div>
                ...
                <div className="flex flex-col gap-6">
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-4">
                    <h3 className="font-bold text-white text-sm flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-amber-400 animate-pulse" /> Conselhos do Tutor IA
                    </h3>
                    ...
                    <div className="flex flex-col gap-2 text-xs text-slate-400 border-t border-slate-800 pt-4 leading-relaxed">
                      <p>💡 <b>Dica de Sucesso:</b> Ao concluir o cronograma, selecione a categoria de estudos no nosso <b>Simulador de Entrevista</b> e faça uma simulação de entrevista interativa para consolidar seus conhecimentos.</p>
                      <button 
                        onClick={() => setActiveTab('interview')}
        ...
        {activeTab === 'interview' && (
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2">
              <h2 className="text-2xl font-extrabold text-white flex items-center gap-2">
                <MessageSquare className="w-6 h-6 text-indigo-400" /> Simulador de Entrevista Técnica (Agent 4)
              </h2>
              <p className="text-slate-400 text-sm">
                Conduza uma simulação de conversa em tempo real com um recrutador virtual técnico de ponta. O agente formulará perguntas desafiadoras baseadas nos critérios de nossa base de conhecimento (RAG) e gerará uma nota final.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-1 flex flex-col gap-6">
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-5">
                  <h3 className="font-bold text-slate-200 text-sm border-b border-slate-800 pb-2">Configurações da Sessão</h3>
                  
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tema Técnico</label>
                    <div className="grid grid-cols-2 gap-1.5">
                      {['React', 'Node.js', 'AWS', 'PostgreSQL', 'System Design', 'Behavioral'].map((cat) => (
                        <button 
                          key={cat}
                          onClick={() => !isInterviewStarted && setInterviewCategory(cat)}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nível de Dificuldade</label>
                    <div className="grid grid-cols-3 gap-1.5">
                      {['Junior', 'Pleno', 'Senior'].map((sen) => (
                        <button 
                          key={sen}
                          onClick={() => !isInterviewStarted && setInterviewSeniority(sen)}
                          disabled={isInterviewStarted}
                  ...
                  <div className="text-[11px] text-slate-500 border-t border-slate-800 pt-4 leading-relaxed">
                    ⚙️ <b>Como Funciona:</b> O bot conduzirá uma conversa com 3 perguntas técnicas do tema e nível selecionados. Ao responder, envie sua resposta em texto. Ao fim, um scorecard completo com nota e feedbacks será liberado automaticamente.
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-3 flex flex-col h-[520px] bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl">
                {isInterviewStarted ? (
                  <div className="flex flex-col h-full">
                    ...
                    <div className="bg-slate-950 px-6 py-3.5 border-b border-slate-850 flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs font-extrabold text-white">Sessão de Simulação Ativa</span>
                      </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 scrollbar-thin">
                      {messages.map((message: { id: string, role: string, content: string }) => {
                        ...
                        if (message.content === "Estou pronto para iniciar a simulação de entrevista técnica.") {
                          return null;
                        }
                        ...
                        return (
                          ...
                          >
                            <div className={`max-w-[85%] md:max-w-[70%] p-4 rounded-2xl flex flex-col gap-1 shadow ${
                              isAssistant 
                                ? 'bg-slate-950 text-slate-100 rounded-tl-none border border-slate-850' 
                                : 'bg-indigo-600 text-white rounded-tr-none'
                            }`}>
                              <span className={`text-[9px] uppercase font-black tracking-wider ${isAssistant ? 'text-indigo-400' : 'text-indigo-200'}`}>
                                {isAssistant ? 'Recrutador IA' : 'Você (Candidato)'}
                              </span>
                      ...
                      {isChatLoading && (
                        <div className="flex justify-start">
                          <div className="bg-slate-950 p-4 rounded-2xl rounded-tl-none border border-slate-850 text-slate-300 flex items-center gap-2">
                            <span className="text-[10px] text-slate-500 font-bold uppercase">Digitando</span>
                    ...
                    {/* INPUT DE MENSAGEM */}
                    <form onSubmit={handleSubmit} className="p-4 bg-slate-950 border-t border-slate-850 flex gap-2">
                      <input 
                        id="chat-input"
                        type="text" 
                        value={input}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center p-8">
                    ...
                    <div className="max-w-md">
                      <h3 className="font-bold text-white text-base">Preparado para Simular?</h3>
                      <p className="text-xs text-slate-500 mt-1">
                        Selecione as configurações desejadas (Categoria de Tecnologia e Dificuldade) na barra lateral esquerda e inicie sua simulação.
                      </p>
                    </div>
                  </div>
                )}
      ...
      <footer id="about" className="border-t border-slate-900 bg-slate-950 px-6 py-12 text-slate-500 text-xs">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="flex flex-col gap-3">
            <h4 className="font-extrabold text-sm text-slate-400">AI Career Coach</h4>
            <p className="leading-relaxed">
              Uma ferramenta inteligente para desenvolvedores que querem decolar em processos seletivos. Mapeie gaps conceituais, execute um motor RAG sob arquivos markdown em tempo real e teste seu nível em simulados automatizados.
            </p>
          </div>
          <div className="flex flex-col gap-3">
            <h4 className="font-extrabold text-sm text-slate-400">Stack de Tecnologias</h4>
            <p className="leading-relaxed">
              Construído com Next.js 15, React 19, TypeScript, TailwindCSS v4, Vercel AI SDK (GPT-4o), PostgreSQL, e pgvector integrado ao banco de dados Supabase para busca semântica em tempo de execução.
            </p>
          </div>
          <div className="flex flex-col gap-3">
            <h4 className="font-extrabold text-sm text-slate-400">Armazenamento Seguro</h4>
            <p className="leading-relaxed">
              Todos os dados e relatórios de vagas e currículos são salvos de forma isolada na tabela do banco, garantindo que o seu histórico e conselhos fiquem salvos de forma resiliente.
            </p>
          </div>
        </div>
        <div className="max-w-7xl mx-auto mt-12 border-t border-slate-900 pt-6 text-center text-[10px]">
          © {new Date().getFullYear()} AI Career Coach. Todos os direitos reservados.
        </div>
      </footer>
